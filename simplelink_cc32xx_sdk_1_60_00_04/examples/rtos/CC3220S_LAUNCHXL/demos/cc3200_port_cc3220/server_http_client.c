...
int8_t myCinHttpConnect(HTTPClient_Handle *pHttpClient, char *address, uint16_t port, uint8_t secure)
{
    uint32_t ulDestinationIP; // IP address of destination server
    int32_t lRetVal = -1, ret;
//    struct sockaddr_in addr;
    ...
    addr.sin_port = sl_Htons(connectPort);

    /* Testing HTTPCli open call: handle, address params only */
    int32_t flag = 0;
    if (secure)
    {
        flag |= HTTPClient_HFIELD_PERSISTENT;
    }
    int16_t statusCode;
    pHttpClient = HTTPClient_create(&statusCode,0);
    if (statusCode < 0)
    {
  	  UART_PRINT("\nhttpTask: creation of http client handle failed\r\n");
    }
    ret = HTTPClient_connect(pHttpClient,&address,0,0);
    if (ret < 0)
    {
    	UART_PRINT("httpTask: connect failed", ret);
    }
//    HTTPCli_construct(pHttpClient);
//    lRetVal = HTTPCli_connect(pHttpClient, (SlSockAddr_t *)&addr, flag, NULL);
//    if (lRetVal < 0)
//    {
...
//    HTTPCli_setRequestFields(pHttpClient, fields);
	HTTPClient_setHeader(httpClientHandle, HTTPClient_HFIELD_REQ_USER_AGENT,USER_AGENT,strlen(USER_AGENT),HTTPClient_HFIELD_PERSISTENT);
    // then send request
    bool moreFlags = 1;
    int32_t lRetVal ;
    lRetVal = HTTPClient_sendRequest(pHttpClient,method,url,msg,strlen(msg),0);
    if (lRetVal < 0) {
    	UART_PRINT("httpTask: send failed",lRetVal);
    }
    if (lRetVal < 0) {
